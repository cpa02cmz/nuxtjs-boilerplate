            ...
            >
              ...
              <div class="mt-2">
                <button
                  ...
                  @click="toggleKeyVisibility(key as ApiKeyDisplay)"
                >
                  {{
                    key.showFullKey
...
.checkmark-path {
  ...
  animation: draw-check
    v-bind('easingConfig?.animations?.checkmarkDraw?.durationMs ?? 400 + "ms"')
    v-bind(
      'easingConfig?.animations?.checkmarkDraw?.easing ?? "cubic-bezier(0.4, 0, 0.2, 1)"'
    )
    forwards;
}
...
.animate-check-pop {
  animation: check-pop
    v-bind('easingConfig?.animations?.checkmarkPop?.durationMs ?? 400 + "ms"')
    v-bind(
      'easingConfig?.animations?.checkmarkPop?.easing ?? "cubic-bezier(0.175, 0.885, 0.32, 1.275)"'
    );
}

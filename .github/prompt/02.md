...
- ❌ Silent error suppression (empty catch)
...
3. **If build fails → that is your ONLY task**
...
**🟡 HIGH** 3. Fix lint errors 4. Fix runtime bugs 5. Extract hardcoded values

**🟢 STANDARD** 6. Strengthen types 7. Add error handling 8. Remove dead code/files 9. Eliminate duplicate code 10. Resolve lint warnings
...
4. **Rollback**: If new issues → revert → document → try safer approach

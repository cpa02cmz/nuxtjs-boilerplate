...
.success-circle {
  ...
  animation: circle-scale
    v-bind('animationConfig.recommendations?.circleScaleDurationSec ?? "0.3s"')
    ease-out
    v-bind('animationConfig.recommendations?.circleScaleDelaySec ?? "0.1s"')
    both;
}
...
.success-checkmark {
  ...
  animation: checkmark-draw
    v-bind(
      'animationConfig.recommendations?.checkmarkDrawDurationSec ?? "0.3s"'
    )
    ease-out
    v-bind('animationConfig.recommendations?.checkmarkDrawDelaySec ?? "0.2s"')
    forwards;
}
...
.animate-refresh-spin svg {
  animation: refresh-spin
    v-bind('animationConfig.recommendations?.refreshSpinDurationSec ?? "0.5s"')
    ease-out;
}

...
const TAGS_DISPLAY_LIMIT = recommendationConfig.displayLimits.tags

const displayTags = computed(() => {
  if (!props.resource?.tags) return []
  return props.resource.tags.slice(0, TAGS_DISPLAY_LIMIT)
})

const hasMoreTags = computed(() => {
  return (props.resource?.tags?.length || 0) > TAGS_DISPLAY_LIMIT
})
</script>

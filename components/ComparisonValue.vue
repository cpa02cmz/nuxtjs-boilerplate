...
.comparison-value--interactive:active {
  transform: translateY(
    ...
    scale(v-bind('animationConfig.cssTransforms.scale.press'));
}
...
@keyframes copy-success-pulse {
  ...
  50% {
    transform: scale(v-bind('animationConfig.cssTransforms.scale.large'));
  }
  100% {
    transform: scale(v-bind('animationConfig.cssTransforms.scale.none'));
  }
...
.comparison-value__icon {
  ...
  align-items: center;
  justify-content: center;
  opacity: v-bind('animationConfig.cssTransforms.opacity.dimmed');
  transition: opacity

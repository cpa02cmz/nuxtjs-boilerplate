...
const toggleLike = (commentId: string, _event?: Event) => {
  if (likedComments.value.has(commentId)) {
    ...
    if (!prefersReducedMotion.value) {
      ...
      }, uiTimingConfig.comments.burstBuffer) // Animation duration + buffer
    }
  }
}
...
.animate-avatar-pulse {
  animation: avatar-pulse
    v-bind('animationConfig?.resourceComments?.avatarPulseSec + "s" || "2s"')
    ease-in-out;
}
...
.live-indicator.animate-pulse {
  animation: live-pulse
    v-bind('animationConfig?.resourceComments?.livePulseSec + "s" || "2s"')

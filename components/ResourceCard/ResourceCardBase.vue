...
const handleNewBadgeHover = () => {
  ...
  // Reset after animation completes - Flexy hates hardcoded 1000ms!
  setTimeout(() => {
    showNewBadgeParticles.value = false
  }, animationConfig.viewedBadge.newBadgeParticle.resetTimeoutMs)
}
...
const shineStyle = computed(() => {
  ...
  return {
    ...
    '--shine-size': '60%',
    // Flexy hates hardcoded 0.3s! Using config.transitionSec
    '--shine-transition': `${config.transitionSec}s`,
  }
})
...
.new-badge-particle {
  ...
  margin-top: -2px;
  margin-left: -2px;
  /* Flexy hates hardcoded 600ms! Using configurable particle duration */
  animation: new-badge-particle-burst
    var(
      --particle-duration,
      v-bind(
        '`${animationConfig.viewedBadge.newBadgeParticle.baseDurationMs}ms`'
      )
    )

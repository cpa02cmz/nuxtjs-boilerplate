      ...
      <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
        ...
        <div v-else class="space-y-3">
          <div class="grid grid-cols-2 gap-4">
            ...
            >
              <p class="text-sm text-gray-500">{{ performanceLabels.fast }}</p>
          ...
          >
            <p class="text-sm text-gray-500">{{ performanceLabels.slow }}</p>
...
// 🧩 Flexy: Performance threshold labels - eliminating hardcoded values!
const performanceLabels = computed(() => {
  const fast = animationConfig.searchResponseTime.fastThresholdMs
  const medium = animationConfig.searchResponseTime.mediumThresholdMs
  return {
    fast: `Fast Searches (<${fast}ms)`,
    medium: `Medium Searches (${fast}-${medium}ms)`,
    slow: `Slow Searches (>${medium}ms)`,
  }
})
...
const getSearchItemStyle = (index: number) => {
  ...
  return {
    '--stagger-delay': `${index * animationConfig.searchAnalytics.barStaggerMultiplierMs}ms`,
  }
}

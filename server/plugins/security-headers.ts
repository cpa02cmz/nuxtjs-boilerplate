...
export default defineNitroPlugin(nitroApp => {
  nitroApp.hooks.hook('render:html', (html, { event }) => {
    // Check if we're in a test environment or if the response object is available
    // This check handles both test environment detection and safe header setting
    if (process.env.NODE_ENV === 'test' || !event?.node?.res) {
      return
    }
    ...
    try {
      ...
      // In test environments, header setting might fail safely
      // We silently handle the error to avoid issues in test environment
    }
  })
})
